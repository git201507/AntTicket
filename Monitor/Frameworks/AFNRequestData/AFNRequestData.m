...
+ (AFNRequestData *)sharedUtil {
    ...
    static AFNRequestData * setSharedInstance;
    
    dispatch_once(&onceToken, ^{
        setSharedInstance = [[AFNRequestData alloc] init];
    ...
    switch (method) {
        case METHOD_GET:
        {
            [manager GET:URLString parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
                ...
                succeed([AFNRequestData dictionaryWithJsonString:responseStr]);
                
//                DNSLog(@"\n 请求成功:%@\n\n",[AFNRequestData dictionaryWithJsonString:responseStr]);
            ...
            failure:(void (^)(NSError *))failure
{
    ...
    URLString = [URLString stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
//    DNSLog(@"\n POST上传多张图片参数列表:%@\n\n%@\n",parameters,[AFNRequestData URLEncryOrDecryString:parameters IsHead:false]);
